...
这个架构彻底消除了"Team 切换"问题——阶段 B 不创建任何 Team，阶段 C 才创建唯一的 Agent Team。Lead 在阶段 B 结束后直接 cd 到项目目录，启动 Agent Team 开始开发。
...
3. 创建 Agent Team，按 CLAUDE.md 中的角色定义 spawn teammates
4. 按 roadmap.md 中的 Phase 顺序推进开发

**重要：** 元仓库的 CLAUDE.md 只定义阶段 A 和 B 的行为。进入阶段 C 后，Lead 完全按项目 CLAUDE.md 行事。元仓库 CLAUDE.md 中不包含任何阶段 C 的具体开发指令。

**上下文压力管理：** 从阶段 A 到阶段 C，Lead 的上下文窗口会逐渐积累。Claude Code 在 ~95% 容量时触发 auto-compaction，之前的对话会被压缩。阶段 B 的所有产出已持久化为文件，Lead 进入阶段 C 后完全依赖读取文件获取上下文，不依赖对话记忆。
